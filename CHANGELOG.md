...
- 项目中心按最终交付日期排序，提升项目较多时的滚动显示。
- 首页交付节点重新排版，清晰显示项目、节点类型和负责人。

### 修复

- 修复 Windows 标题栏按钮、窗口拖动、滚动条及窗口边缘显示问题。
- Windows 重复双击应用时改为唤起已打开窗口，不再重复打开。
- 修复部分页面滚动后无法拖动窗口及首页底部出现大块空白的问题。
...
- 团队模式显示当前在线人数，并可在数据保存位置中查看在线成员、账号角色和连接设备数量。

### 优化

- 财务账号或项目经理账号在本人被指定为项目负责人后，可编辑自己负责的项目；其他项目仍保持只读。
- 归档项目的负责人可在归档页调整项目健康度、当前状态、流程节点和最终交付日期，不开放删除、改名、换负责人或修改任务。
- CrewFlow 助理默认收起为右下角小图标，减少对页面内容的遮挡。
- 合同文件路径改为更紧凑清晰的独立行布局。

### 修复

- 团队客户端更新或启动后会每 2 秒自动恢复连接并重新载入团队数据，无需先手动点击“测试连接”。
- 团队数据尚未真正载入时暂停新增和修改，避免界面显示已连接但数据没有同步。
- 团队保存状态和最近同步时间在真实读写完成后更新，便于确认修改是否已经保存。
...
- 首页控制台的交付节点显示所属项目名称，便于区分同名或相近节点。
